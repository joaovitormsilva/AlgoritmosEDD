/*
Estrutura para associar uma chave a um valor
 Listas lineares: operações de busca e remoção de uma lista são comumente feitas em relação à chave
 do elemento.
 Ordenada: Deve-se buscar a posição correta
 Não-ordenada: insere-se no fim ou no início.
 
 TAD - LISTAS:
 Básicas: Criar lista vazia -> inserir elemento -> pesquisar elemento -> remover elemento -> verificar se está vazia -> se está cheia
 
 Operações comuns: Contar números de elementos, recuperar por posição, primeiro/cabeça ou último/cauda
 Imprimir todos os elementos, Concatenar listas
